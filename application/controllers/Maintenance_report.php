...
class Maintenance_report extends Pre_loader
{
 ...
 public function preventive(){
    ...
    $this->template->rander("maintenance/reports/selected",$view_data);
}

public function reactive(){
    $view_data['preventives']=$this->db->query("SELECT * FROM jobs WHERE reactive=1")->result_array();
    $this->template->rander("maintenance/reports/selected",$view_data);
}
...
public function view($month){
   ...
   $query=$this->db->query("SELECT * FROM jobs")->result_array();
   $date=[];
...
public function print_page($month){
   $query=$this->db->query("SELECT * FROM jobs")->result_array();
   $date=[];
    foreach ($query as $key => $value) {
      $conv=strtotime($value['created_at']);
