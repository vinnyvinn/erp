...
class Estimates extends Pre_loader {
    ...
    function save() {
        ...
        $estimate_data = array(
            ...
            "valid_until" => $this->input->post('estimate_date'),
    ...
    private function _make_row($data) {
        $estimate_url = "";
        if ($this->login_user->user_type == "staff") {
            $estimate_url = anchor(get_uri("estimates/view/" . $data->id), lang("estimate") . " - " . $data->id);
        } else {
            //for client client
            $estimate_url = anchor(get_uri("estimates/preview/" . $data->id), lang("estimate") . " - " . $data->id);
        }
        ...
        return array(
            ...
            $data->estimate_date,
            format_to_date($data->estimate_date),
    ...
    private function _prepare_estimate($estimate_id, $mode = "download", &$data = array()) {
        ...
        if ($estimate_id) {
            ...
            if ($mode === "download") {
                $this->pdf->Output("Estimate-$estimate_id.pdf", "D");
            } else if ($mode === "send_email") {
                $temp_download_path = getcwd() . "/" . get_setting("temp_file_path") . "Estimate-$estimate_id.pdf";
                $this->pdf->Output($temp_download_path, "F");
                return $temp_download_path;
            } else if ($mode === "view") {
                $this->pdf->Output("Estimate-$estimate_id.pdf", "I");
            } else if ($mode === "html") {
                return $html;
            }
        }
