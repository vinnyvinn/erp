...
class Invoice_items_model extends Crud_model {
    ...
    function get_item_suggestion($keyword = "") {
        ...
        FROM $invoice_items_table
        WHERE $invoice_items_table.deleted=0  AND $invoice_items_table.title LIKE '%$keyword%'
        GROUP BY $invoice_items_table.title LIMIT 5 )
        UNION
        (SELECT $estimate_items_table.title
        FROM $estimate_items_table
        WHERE $estimate_items_table.deleted=0  AND $estimate_items_table.title LIKE '%$keyword%'
        GROUP BY $estimate_items_table.title LIMIT 5)
        ";
