<form method="POST" action="<?php echo base_url('parts_requisition/save_part') ?>">
  ...
  <div class="col-sm-3">
    <div class="form-group">
      ...
      <input type="number" name="qnty_out" id="quantity" class="form-control" required="required" onkeyup="checkQuantity()">
...
<script type="text/javascript">
 $(document).ready(function() {
   $('select[name="stock_id"]').on('change', function() {
    ...
    var path="<?php echo site_url('parts_requisition/displayParts')?>/" + part_id;
    $.ajax({
      type  : 'ajax',
      url   : path,
      async : false,
      dataType : 'json',
      success : function(data){
        localStorage.setItem('qn',data);
        if(data < 1){
         alert('sorry item quantity  is less than 1');
         window.location.replace('createPart');
 ...
 function checkQuantity(){
  var item=localStorage.getItem('qn');
  var qnty=document.getElementById('quantity');
  if(item < qnty.value){
    alert('sorry we do not have enough items in store');
    window.location.replace('createPart');
  }
